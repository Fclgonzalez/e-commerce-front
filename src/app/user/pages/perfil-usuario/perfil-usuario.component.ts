...
export class PerfilUsuarioComponent implements OnInit {
 ...
 formUser: FormGroup = this.fb.group({
  ...
  ngOnInit(): void {
      ...
      this.userService.atualizarUsuarioSub$.subscribe(
        (atualizar) => {
          if (atualizar) {
      ...
      this.enderecoService.atualizarEndereco$.subscribe(
        (atualizar) => {
          if (atualizar) {
    ...
    recuperarEnderecoUsuario() {
        ...
        this.userService.getUsuarioByUsername(id).subscribe(
          (user)=>{
            this.enderecoService.getEnderecosById(user.idUser!).subscribe(
              (endereco) => {
    ...
    valorMudouUser() {
      ...
      .subscribe(
        (valores) => {
          this.desabilitar = this.formUser.invalid || !(valores.nome != this.user.nome || valores.username != this.user.username || valores.celular != this.user.celular || valores.telefone != this.user.telefone )
      })
    }
    ...
    valorMudouEnderecoUsuario() {
      ...
      .subscribe(
        (valores) => {
          this.desabilitar = this.formEndereco.invalid || !(valores.logradouro != this.endereco.logradouro || valores.numero != this.endereco.numero || valores.complemento != this.endereco.complemento || valores.cep != this.endereco.cep
          || valores.bairro != this.endereco.bairro || valores.cidade != this.endereco.cidade || valores.uf != this.endereco.uf)
      })
    } 
      ...
      atualizarUser():void {
        ...
        this.userService.patchUsuario(u.username, u)
        .subscribe(
          (usuario) => {
       ...
       atualizarEndereco(): void {
         ...
         this.enderecoService.atualizarEndereco(e)
         .subscribe(
           (endereco) => {
             this.snackbar.open('Alterações Salvas com Sucesso!', 'Ok', {
               duration: 3000
